...
struct ResetData{
	unsigned int MemSize;		//RAM size in MB.
	unsigned int BootMode;		//The boot mode (0 = hard reset, 1 = soft reset, 2 = update reset, 3 = update complete).
	const char *command;		//Updater command line (Only for when boot mode = 2).
	void* StartAddress;		//Pointer to the entry point of the first module.
	void* IOPRPBuffer;
	unsigned int IOPRPBufferSize;
	unsigned int NumModules;
	const void **ModData;		//Pointers to each selected module that will be loaded.
};

#ifdef FULL_UDNL
static int var_00001850=0;	//Not sure what this is, but it's probably a verbosity level flag, although unused.
#endif
...
static void LoadIRXModule(const void *module, struct ModuleInfo *ModuleInfo){
	...
	for(SectionNum=0,CurrentELF_shdr=ELF_shdr+1; SectionNum<ELF_hdr->shnum; SectionNum++,CurrentELF_shdr++){
		if(CurrentELF_shdr->type==SHT_REL){
			...
			//Warning: beware of sign extension! The code here depends on sign extension.
...
//Initializes the module information structure, which exists 0x30 bytes before the module itself.
...
//Scans through all loaded IOPRP images for the newest version of the specified module.
...
//This function hates me and I hate it.
