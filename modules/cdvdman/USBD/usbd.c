...
#
# $Id: usbd.c 1682 2010-05-18 17:44:20Z jim $
...
/* Shared semaphore creation data */
iop_sema_t sema={
	1,	/* sema.attr */
	0,	/* sema.option */
	1,	/* sema.initial */
...
/* Shared event flag creation data */
iop_event_t event={
	0,	/* attr */
	0,	/* option */
	0	/* bits */
...
int doGetDeviceLocation(Device *dev, uint8 *path) {
	...
	if (dev == memPool.deviceTreeRoot) {
		for (cpCount = 0; cpCount < 7; cpCount++) {
			path[cpCount] = ((cpCount < count)?tempPath[count-(cpCount+1)]:0);
		}
		return 0;
	} else
...
void processDoneQueue_IsoTd(HcIsoTD *arg) {
	...
	if ((tdHcRes == USB_RC_OK) && ((pswRes == USB_RC_OK) || (pswRes == USB_RC_DATAUNDER))) {
		req->transferedBytes = (((req->correspEndpoint->hcEd.hcArea & HCED_DIR_MASK) == HCED_DIR_IN)?pswOfs:req->length);
	}

	req->prev = listEnd;
	if(listEnd) listEnd->next = req;
	else listStart = req;
...
void processDoneQueue_GenTd(HcTD *arg) {
	...
	if ((req = memPool.hcTdToIoReqLUT[arg - memPool.hcTdBuf])) {
		...
		if (arg->bufferEnd && (tdHcArea & 0x180000)) { // dir != SETUP
			req->transferedBytes=((arg->curBufPtr==0)?req->length:((uint8 *)arg->curBufPtr-(uint8 *)req->destPtr)); /* if arg->curBufPtr == 0, transfer was successful. */
		}
		hcRes = tdHcArea >> 28;
		freeTd(arg);
		...
		if (hcRes && ED_HALTED(req->correspEndpoint->hcEd)) {
			...
			while (tdListPos && (tdListPos != ed->tdTail)) {
				...
				if (req) {
					...
					IoRequest *listPos;
					for (listPos = firstElem; listPos != NULL; listPos = listPos->next) if(listPos==req) break;
		...
		while (pos) {
			...
			if (dev) {
				if (pos->callbackProc) pos->callbackProc(pos);
			} else
				freeIoRequest(pos);
			pos = next;
		}
...
inline void handleTimerList(void) {
	...
	if (timer) {
		...
		while (memPool.timerListStart && (memPool.timerListStart->delayCount == 0)) {
			...
			if(timer->next) timer->next->prev = NULL;
			else memPool.timerListEnd = NULL;
...
void initUSBD(void){ /* Initilze USBD */
	...
	RegisterLibraryEntries(&_exp_usbd);
