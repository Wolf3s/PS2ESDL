...
static UsbDriver *drvListStart = NULL, *drvListEnd = NULL;
static IoRequest *cbListStart = NULL, *cbListEnd = NULL;
...
int doRegisterDriver(UsbDriver *drv, void *drvGpSeg){
	if(drv->next||drv->prev||(drvListStart==drv)) return USB_RC_BUSY;
...
static void callbackThreadFunc(void *arg){
	...
	IoRequest reqCopy;
	while (1) {
		WaitEventFlag(callbackEvent, 1, WEF_CLEAR | WEF_OR, NULL);
		do {
			CpuSuspendIntr(&intrStat);
