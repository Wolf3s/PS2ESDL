...
#
# $Id: hub.c 1682 2010-05-18 17:44:20Z jim $
...
inline int initHubDriver(void){
	...
	hubBufferList = (UsbHub *)malloc(needMem);
	if (!hubBufferList) {
...
static UsbHub *allocHubBuffer(void) {
...
static void freeHubBuffer(UsbHub *hub) {
	if (hub) {
...
static void HubControlTransfer(UsbHub *hubDev, uint8 requestType, uint8 request, uint16 value,
...
static void hubGetHubStatusCallback(IoRequest *req);
static void hubGetPortStatusCallback(IoRequest *req);
static void getHubStatusChange(UsbHub *dev);
...
static int cancelTimerCallback(TimerCbStruct *arg) {
	if (arg->isActive) {
		if(arg->next) arg->next->prev = arg->prev;
		else memPool.timerListEnd = arg->prev;

		if(arg->prev) arg->prev->next = arg->next;
		else memPool.timerListStart = arg->next;
...
static void killEndpoint(Endpoint *ep) {
	int i = 0;
	IoRequest *req;
	HcED *hcEd = &ep->hcEd;
	...
	while ((req = ep->ioReqListStart)) {
		...
		else ep->ioReqListStart = req->next;
	...
	if(memPool.freeEpListEnd) memPool.freeEpListEnd->next = ep;
	else memPool.freeEpListStart = ep;
...
int removeEndpointFromDevice(Device *dev, Endpoint *ep) {
	...
	else dev->endpointListEnd = ep->prev;

	if(ep->prev) ep->prev->next = ep->next;
	else dev->endpointListStart = ep->next;
...
static void hubDeviceResetCallback(IoRequest *arg) {
...
static int checkDelayedResetsTree(Device *tree) {
	Device *dev;
...
static void checkDelayedResets(Device *dev) {
	memPool.delayResets = 0;
	dev->resetFlag = 0;
	checkDelayedResetsTree(memPool.deviceTreeRoot);
}

static void killDevice(Device *dev, Endpoint *ep) {
...
static void fetchConfigDescriptors(IoRequest *req) {
	...
	if ((req->resultCode == USB_RC_OK) || (dev->fetchDescriptorCounter == 0)) {
		...
		if (curDescNum < ((UsbDeviceDescriptor*)dev->staticDeviceDescPtr)->bNumConfigurations) {
			...
			if(memPool.deviceConnectedListEnd) memPool.deviceConnectedListEnd->nextConnected = dev;
			else memPool.deviceConnectedListStart = dev;
...
static void requestDeviceDescriptor(IoRequest *req, uint16 length);

static void requestDevDescrptCb(IoRequest *req) {
	Endpoint *ep = req->correspEndpoint;
	Device *dev = ep->correspDevice;
...
static void hubPeekDeviceDescriptor(IoRequest *req) {
...
static void hubSetFuncAddress(Endpoint *ep);

static void hubSetFuncAddressCB(IoRequest *req) {
	Endpoint *ep = req->correspEndpoint;
	Device *dev = ep->correspDevice;
	...
	//	addTimerCallback(&dev->timer, (TimerCallback)&hubPeekDeviceDescriptor, req, 10);
		addTimerCallback(&dev->timer, (TimerCallback)&hubPeekDeviceDescriptor, req, 240);
	}
}
...
static void hubSetPortPower(IoRequest *req) {
	UsbHub *dev = (UsbHub *)req->userCallbackArg;
...
static void hubSetupPorts(IoRequest *req) {
	int port;
	UsbHub *dev = (UsbHub *)req->userCallbackArg;
...
static void hubCheckPorts(IoRequest *req) {
	UsbHub *dev = (UsbHub *)req->userCallbackArg;
	if (req->resultCode == USB_RC_OK) {
...
static void hubCheckDeviceDesc(IoRequest *req) {
	UsbHub *dev = (UsbHub *)req->userCallbackArg;
	if (req->resultCode == USB_RC_OK) {
...
static int hubDrvProbe(int devId) {
	...
	return((devDesc && (devDesc->bDeviceClass == USB_CLASS_HUB) && (devDesc->bNumConfigurations == 1))?1:0);
}
