...
#
# $Id: usbio.c 1682 2010-05-18 17:44:20Z jim $
...
static inline int setupControlTransfer(Endpoint *ep) {
	...
	if (ep->hcEd.tdTail && !ED_HALTED(ep->hcEd) && !ED_SKIPPED(ep->hcEd) && curIoReq) {
		...
		if(curIoReq->next) curIoReq->next->prev = curIoReq->prev;
		else ep->ioReqListEnd = curIoReq->prev;

		if(curIoReq->prev) curIoReq->prev->next = curIoReq->next;
		else ep->ioReqListStart = curIoReq->next;
		...
		if (dataTd) {
			...
			dataTd->HcArea = ((curIoReq->devReq.requesttype & USB_DIR_IN)?(TD_HCAREA(USB_RC_NOTACCESSED, 3, 7,  TD_IN, 1)):(TD_HCAREA(USB_RC_NOTACCESSED, 3, 7, TD_OUT, 1))) << 16;
		...
		// third stage: status
		statusTd->HcArea = ((curIoReq->devReq.requesttype & USB_DIR_IN)?(TD_HCAREA(USB_RC_NOTACCESSED, 3, 0, TD_OUT, 0)):(TD_HCAREA(USB_RC_NOTACCESSED, 3, 0,  TD_IN, 0))) << 16;
...
static inline int setupIsocronTransfer(Endpoint *ep) {
	IoRequest *curIoReq = ep->ioReqListStart;
	...
	if (ep->hcEd.tdTail && !ED_HALTED(ep->hcEd) && !ED_SKIPPED(ep->hcEd) && curIoReq) {
		...
		if(ed->tdTail == (HcTD *)((uint32)ed->tdHead & ~0xF)) frameNo = memPool.hcHCCA->FrameNumber + 2;
		else frameNo = ep->isochronLastFrameNum;
		...
		if(curIoReq->destPtr && curIoReq->length) curTd->bufferEnd = (uint8*)curIoReq->destPtr + curIoReq->length - 1;
		else curTd->bufferEnd = NULL;
...
static inline int setupBulkTransfer(Endpoint *ep) {
	IoRequest *curIoReq = ep->ioReqListStart;
...
void handleIoReqList(Endpoint *ep) {
	if(ep->endpointType == TYPE_CONTROL) setupControlTransfer(ep);
	else if(ep->endpointType == TYPE_ISOCHRON) setupIsocronTransfer(ep);
	else setupBulkTransfer(ep); /* Either bulk or interrupt transfer type. */
}

int attachIoReqToEndpoint(Endpoint *ep, IoRequest *req, void *destdata, uint16 length, void *callback) {
	if(!ep->correspDevice) return USB_RC_BUSY;
	if(req->busyFlag) return USB_RC_BUSY;
	...
	if(ep->ioReqListEnd) ep->ioReqListEnd->next = req;
	else ep->ioReqListStart = req;
