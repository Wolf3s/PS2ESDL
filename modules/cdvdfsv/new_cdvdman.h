...
typedef struct {
	...
	u8 minute;
	u8 hour;
	u8 padding; /* This is really just a padding field. */
	u8 day;
	u8 month;
	u8 year;
} cd_clock_t;
...
typedef struct {
	...
	u32 flag;       /* Bits 0-7: ISO9660 file type flag, Bits 8-31 are reserved */          
} adv_cd_file_t;
...
int sceCdReadDVDV(u32 lsn,u32 sectors,void *buf,cd_read_mode_t *mode);  /* SP193: Corrected "int"s to "u32"s in prototype */
...
int sceCdReadKey(int arg1, int arg2, u32 lsn, char *key);
...
int sceCdReadConsoleID(u64 *id, u32 *stat);                     /* SP193: Changed "u32 *res, int *stat" to "u64 *id, u32 *stat" */
#define I_sceCdReadConsoleID DECLARE_IMPORT(41, sceCdReadConsoleID)

/* SP193: Added export #43 */
int sceCdMV(u32 *mv,u32 *stat);
...
int sceCdReadSUBQ(char *buf,u32 *stat);
...
int sceCdStRead(u32 size,void *buf,u32 mode,u32 *err);
...
int sceCdRV(u32 lsn, u32 sectors, void *buf, cd_read_mode_t *mode, int csec, void *cb);
...
int sceCdReadFull(u32 lsn,u32 sectors,void *buf,cd_read_mode_t *mode);
...
int sceCdReadModelID(u32 *id);                                  /* SP193: Note! DNAS IOPRP images only! */
...
int sceCdApplySCmd2(u8 cmd,void *wdata,u32 wdlen,void *rdata);
#define I_sceCdApplySCmd2 DECLARE_IMPORT(112, sceCdApplySCmd2)
int sceCdReadIOPm(u32 lsn,u32 sectors,void *buf,cd_read_mode_t *mode);
