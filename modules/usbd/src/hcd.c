...
#
# $Id: hcd.c 1682 2010-05-18 17:44:20Z jim $
...
static int hcdIrqEvent; /* HCD IRQ event flag. */

static int cleanUpFunc(Device *dev, Endpoint *ep) {
	if(!ep) return 0;

	if((ep < memPool.endpointBuf) || (ep >= memPool.endpointBuf + usbConfig.maxEndpoints))
		return 0;

	if(ep->inTdQueue) removeEndpointFromQueue(ep);
...
Endpoint *openDeviceEndpoint(Device *dev, UsbEndpointDescriptor *endpDesc, uint32 alignFlag) {
	...
	if (endpDesc) {
		...
		hcMaxPktSize &= 0x7FF;
		if(type == USB_ENDPOINT_XFER_ISOC) flags |= HCED_ISOC;
		if(dev->isLowSpeedDevice) flags |= HCED_SPEED;

		flags |= (endpDesc->bEndpointAddress & 0xF) << 7;

		if(endpDesc->bEndpointAddress & USB_DIR_IN) flags|= HCED_DIR_IN;
		else flags|= HCED_DIR_OUT;
		...
		newEp->hcEd.hcArea=(dev->isLowSpeedDevice)?HCED_SPEED:0;
		endpType = TYPE_CONTROL;
	}
...
Endpoint *doOpenEndpoint(Device *dev, UsbEndpointDescriptor *endpDesc, uint32 alignFlag) {
	if (!dev->parent) return NULL;

	return((endpDesc == NULL)?dev->endpointListStart:openDeviceEndpoint(dev, endpDesc, alignFlag)); /* If endpDesc==NULL, default control EP was already opened. */
}

int doCloseEndpoint(Endpoint *ep) {
	Device *dev = ep->correspDevice;

	return((dev->endpointListStart!=ep)?removeEndpointFromDevice(dev, ep):0);
}
...
void *doGetDeviceStaticDescriptor(int devId, void *data, uint8 type) {
	...
	descBuf = (UsbDeviceDescriptor *)((data)?((uint8 *)data + ((UsbDeviceDescriptor*)data)->bLength):dev->staticDeviceDescPtr);

	if(type == 0) return descBuf;
...
static inline void handleRhsc(void) {
	uint32 portNum = 0;
...
static inline void hcdProcessIntr(void) {
	uint32 intrFlags;
...
static void hcdIrqThread(void *arg) {
	while (1) {
		WaitEventFlag(hcdIrqEvent, 1, WEF_CLEAR | WEF_OR, NULL);
...
static inline int initHcdStructs(void){
	int		i;
	HcCA	*hcCommArea;
	...
	*(volatile uint32*)0xBF801680 = 1;
	...
	uint8 *memBuf			= AllocSysMemory(ALLOC_FIRST, memSize, NULL);
...
inline int hcdInit(void){
	...
	if (RegisterIntrHandler(IOP_IRQ_USB, 1, &usbdIntrHandler, (void *)hcdIrqEvent) != 0) {
		if (irqRes == IOP_IRQ_USB) EnableIntr(IOP_IRQ_USB);
		return 1;
	}
